...
| `strapi develop` crashes in the admin (often prism `Cannot convert undefined or null to object`, raw `/node_modules/...` module URLs) but `strapi build` + `strapi start` works | Project `src/admin/vite.config.ts` sets `optimizeDeps.entries`, which REPLACES Vite's default entry scan (Strapi sets none) — the stock admin graph misses the pre-bundle | REMOVE the `entries` override (plugin >= 0.8.4 warns about it at startup); to pre-bundle heavy lazy-loaded deps, list those packages in `optimizeDeps.include` instead |
