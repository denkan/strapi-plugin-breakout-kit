...
function headlessContentManager() {
  ...
  /** @type {string | undefined} */ let shimTargetPath;

  const resolveRoots = (rootDir) => {
    cmRoot = packageRoot('@strapi/content-manager', rootDir);
    shimTargetPath = path.join(cmRoot, 'dist', 'admin', 'hooks', 'useDocumentContext.mjs');
  };

  const shimPath = path.join(RUNTIME_DIR, 'useDocumentContext.mjs');
  ...
  const esbuildResolver = {
    ...
    setup(build) {
      build.onResolve({ filter: /^@strapi\/(content-manager|admin)\/dist\// }, (args) => {
        ...
        const root = args.path.startsWith('@strapi/content-manager')
          ? '@strapi/content-manager'
          : '@strapi/admin';
        const abs = path.join(packageRoot(root, args.resolveDir), args.path.slice(root.length + 1));
        return { path: abs === shimTargetPath ? shimPath : abs };
      });
      build.onResolve({ filter: /useDocumentContext\.mjs$/ }, (args) => {
        if (!args.path.startsWith('.')) return null;
        if (!cmRoot) resolveRoots(process.cwd());
        const abs = path.resolve(args.resolveDir, args.path);
        return abs === shimTargetPath ? { path: shimPath } : null;
      });
    },
  };
    ...
    resolveId(source, importer) {
      ...
      for (const root of DEEP_ROOTS) {
        if (source.startsWith(`${root}/dist/`)) {
          ...
          return abs === shimTargetPath ? shimPath : abs;
        }
      }

      // 2. Relative imports inside @strapi/content-manager that resolve to the module we
      //    replace — every internal consumer gets the shim, keeping one module instance.
      if (
        importer &&
        (source.startsWith('./') || source.startsWith('../')) &&
        source.endsWith('useDocumentContext.mjs')
      ) {
        const abs = path.resolve(path.dirname(importer.split('?')[0]), source);
        if (abs === shimTargetPath) {
          return shimPath;
        }
      }
